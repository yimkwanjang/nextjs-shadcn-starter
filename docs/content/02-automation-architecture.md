...
> 상태: **Draft** — 13절 열린 질문이 확정되면 PRD로 승격한다.
> 작성일: 2026-08-31
...
2. `PROOF` 단계의 `visual.humanRequired`는 **항상 `true`**. 리뷰·인증·판매량을
   AI가 지어내면 그건 자동화가 아니라 허위광고다(14절 참조).
...
문서 서식 취향이 아니라 **파이프라인 설계상 이득**이다. 원문 자신이 그렇게 말한다:

> **"이 표 하나만 있으면, 촬영과 편집은 단순 작업(Simple Task)으로 바뀐다."**

마스터 프롬프트의 `[출력 형식]`이 요구하는 열이 곧 타입의 필드다 —
`구간 | 시간 | 화면 설명(구체적으로) | 나레이션/자막`.

```ts
export interface Shot {
  section: "HOOK" | "RETENTION" | "CTA";  // 표의 '구간' 열
  atSec: number;
  durationSec: number;
...
`visual`은 **"구체적으로"** 써야 한다고 프롬프트가 못 박는다. "곰팡이를 보여준다"가 아니라
**"샤워실 실리콘 틈, 검은 곰팡이 클로즈업"** 이어야 그대로 생성 프롬프트가 된다.
이 구체성이 곧 영상 품질이므로, 추상적인 `visual`은 **스키마 검증 단계에서 되돌린다.**

**불변식**: `sum(hook.durationSec) <= 3`. 훅이 3초를 넘으면 렌더를 거부한다 —
원문이 0~3초를 **이탈 방지 구간**이라 부르는 이유가 여기다.

> **총 길이는 15초를 기준으로 한다.** 원문의 예시 표는 9초로 끝나지만
> `[구조]` 지시문과 3단 공식이 모두 15초를 말하므로 그쪽을 채택했다(04 문서 U-20).
> `totalSec`은 상수가 아니라 설정으로 두어 Q-11이 확정되면 갈아 끼운다.
...
## 9. 올인원 — 브랜드 보이스와 프롬프트 자산

07 문서(14장)는 12장 슈퍼 프롬프트의 완성판이지만, **12장과 다른 모델을 제시한다.**
그 충돌부터 정리해야 나머지가 정리된다.

### 9.1 올인원(병렬) vs 폭포수(압축) — 입력의 두께가 가른다

| | 12장 = 7절 폭포수 | 14장 = 올인원 |
|---|-----------------|--------------|
| 소스 | 블로그 글·상세페이지 (**두꺼움**) | 제품 정보 **3줄** (얇음) |
| 방향 | 코어 → 파생 (**압축**) | 소스 → 4채널 (**팽창**) |
| 속도 | 코어를 먼저 만들어야 함 | **1분** |
| 사실 검증 | **1곳**(코어) | **4곳** — 채널마다 지어낸 살이 붙는다 |
| 잘 맞는 때 | 정보가 많다 · 장기 운영 | **신상품 런칭** · 아직 코어가 없다 |

**어느 하나가 틀린 게 아니라, 제품의 나이가 다르다.**
막 출시한 제품은 스펙 3줄이 전부라 압축할 코어 자체가 없다. 그때 폭포수를 고집하면
"코어를 먼저 쓰세요"라는 말밖에 못 한다 — 반나절이 다시 돌아온다.

→ **두 모드를 다 두고 조건으로 가른다.**

```ts
export type GenerationMode =
  | "ALL_IN_ONE"   // 얇은 소스 → 4채널 병렬 팽창. 빠름. 검증 4곳
  | "WATERFALL";   // 두꺼운 코어 → 압축. 정확. 검증 1곳
```

**운영 규칙**: **1회차는 올인원, 2회차부터 폭포수.**
올인원으로 만든 블로그 글이 사람 손을 거쳐 발행되면 **그게 곧 코어가 된다.**
그다음부터는 그 코어에서 압축해 내려보낸다. 올인원은 **코어를 만드는 부트스트랩**이지
정상 운영 모드가 아니다.

> ⚠️ 올인원 모드에서는 **검증 지점이 1곳으로 수렴하지 않는다**(7절의 핵심 이득이 사라진다).
> 그래서 올인원 산출물은 **전부 초안 상태**로만 두고, 사람이 승인한 하나가 코어로 승격되기
> 전까지는 파생 발행을 막는다.

### 9.2 슈퍼 프롬프트는 한 번에, 호출은 쪼갠다 (7.5 유지)

14장은 "엔터 한 번에 4명의 전문가 몫"을 강조한다. **효과의 출처를 오해하면 안 된다.**
그 효과는 *한 번의 호출*이 아니라 **하나의 소스 + 하나의 페르소나**에서 나온다.
그건 분할 호출로도 그대로 얻어진다 — 오히려 9.3처럼 페르소나를 공통 주입하면 **더 안전하다.**

7.5절의 판단(품질 저하·전체 재생성·캐싱)을 유지한다. 다만 **채널 지시문은 한 파일에
올인원 형태 그대로** 보관한다. 사람이 읽을 때는 슈퍼 프롬프트 한 장으로 보이고,
실행할 때만 쪼개진다.

### 9.3 브랜드 보이스는 단일 주입점을 갖는다

14장 5절이 이 시스템에서 가장 값싸고 효과가 큰 조치를 짚는다.

> "너는 차분하고 신뢰감을 주는 30대 여성 에디터야." — 한 번만 입력해두면 모든 결과물에 깃든다.

**채널별 프롬프트에 각자 톤을 적으면 반드시 어긋난다.** 사람이 기분 따라 흔들리는 것과
똑같은 일이 프롬프트 파일 4개 사이에서 일어난다.

```ts
/** 모든 생성 호출의 시스템 프롬프트에 무조건 앞에 붙는다 */
export interface BrandVoice {
  persona: string;        // "차분하고 신뢰감을 주는 30대 여성 에디터"
  tone: string;           // 해요체 / 반말 / 존댓말
  /** 이미지 생성에도 같은 목소리를 건다 */
  visualTone: string;     // "soft lighting, warm atmosphere, pastel tone"
  forbidden: string[];    // 쓰지 않을 표현
}
```

**불변식**: 생성 함수는 `BrandVoice`를 인자로 **요구**한다. 옵션이 아니다.
채널 프롬프트가 톤을 직접 서술하는 것을 금지하고, 리뷰에서 잡는다.

> 단 **UGC 반말 숏츠**(6.4)는 페르소나가 의도적으로 달라지는 예외다.
> `BrandVoice.persona`를 덮어쓰는 게 아니라 `channelOverride`로 명시하고,
> 광고 표기 의무가 따라붙는다는 사실을 함께 기록한다.

### 9.4 프롬프트 자산 — 노션·엑셀보다 저장소가 낫다

14장 6절의 '황금 프롬프트 = 고정값(Preset) = 무형 자산'은 그대로 받는다.
다만 **저장 위치는 다르게 판단한다.**

| | 노션/엑셀 (원문 권장) | **저장소 `prompts/`** |
|---|---------------------|---------------------|
| 대상 | 비개발자 | 이미 git이 있는 우리 |
| 변경 이력 | 없음 | **커밋 단위로 남는다** |
| "왜 바꿨나" | 기억에 의존 | 커밋 메시지 + 성과 데이터 연결 |
| 되돌리기 | 수동 | `git revert` |

프롬프트는 **성과가 붙는 코드**다. `--stylize 250`을 붙였더니 CTR이 올랐다면,
그 변경과 그 성과가 같은 이력에 남아야 다음 판단을 할 수 있다(8절 피드백 루프).

```
src/lib/persuasion/prompts/
  brand-voice.ts        # 9.3 단일 주입점
  presets.ts            # 황금 프롬프트 고정값 (--stylize 250 등)
  channels/
    blog.ts  instagram.ts  midjourney.ts  shorts.ts  thread.ts
  osmu-super-prompt.md  # 사람이 읽는 올인원 원본 (실행은 쪼갬, 9.2)
```

> **"사람에 의존하지 않고 시스템에 의존한다"** 는 원문의 결론은 8.3절(광고 판정 규칙을
> 코드로 고정)과 같은 사상이다. 이 시스템에서 반복되는 유일한 원칙이라고 봐도 된다 —
> **판단은 사람이 한 번 하고, 그 판단을 코드에 박는다.**

### 9.5 출력이 두 종류라는 사실을 타입이 알아야 한다

올인원 프롬프트의 첫 문장이 명시한다 — *"콘텐츠 원고와 **다른 AI 도구를 작동시키기 위한
명령어**를 한 번에 작성해줘."*

| 종류 | 예 | 소비자 | 검증 방법 |
|------|-----|-------|----------|
| **원고** (Copy) | 블로그 개요, 인스타 캡션 | **사람** | 읽어서 판단 |
| **명령어** (Instruction) | 미드저니 영어 프롬프트, 릴스 Visual\|Audio 표 | **기계** | **스키마 검증** |

```ts
export type Output =
  | { kind: "COPY"; channel: Channel; text: string }
  | { kind: "INSTRUCTION"; target: "MIDJOURNEY" | "VIDEO" | "TTS" | "CANVA";
      payload: unknown };  // target별 스키마로 파싱
```

**명령어는 사람이 읽고 "괜찮네" 할 대상이 아니다.** 파싱에 실패하면 그 자리에서 재생성한다.
지금까지 다섯 번 나온 패턴(Shot 한 줄·카드 표 한 행·미드저니 프롬프트·CardRow·Visual|Audio 표)이
전부 이 `INSTRUCTION` 갈래다.
...
| R25 | **올인원 모드의 팽창을 검증 없이 발행** — 검증 지점이 4곳으로 흩어짐 | 올인원 산출물은 전부 초안. 승인된 하나가 코어로 승격되기 전까지 파생 발행 차단(9.1) |
| R26 | 채널 프롬프트마다 톤을 따로 적어 **브랜드 목소리가 갈라짐** | `BrandVoice` 단일 주입점을 인자로 강제. 채널 프롬프트의 톤 서술 금지(9.3) |
| R27 | 황금 프롬프트가 **노션·엑셀에 흩어져 변경 이력이 사라짐** | `prompts/`에 코드로 두고 커밋 이력 + 성과 데이터를 연결(9.4) |
...
| Q-10 | **영상 트랙과 블로그 트랙 중 무엇을 먼저 굴리는가?** | 10절 순서. 블로그가 더 싸고 빠르지만 성과 확인에 2주 필요 |
...
| Q-20 | **브랜드 페르소나 한 줄**을 무엇으로 확정하는가? | 9.3 `BrandVoice`. 모든 채널·이미지 톤이 여기서 나온다 |
| Q-21 | 이미지 생성 도구는? (미드저니 / 나노 바나나 / 이 환경의 MCP 도구) | 원문은 미드저니 영어 프롬프트 전제. 도구가 바뀌면 `INSTRUCTION` 스키마가 바뀐다(9.5) |
| Q-22 | 신제품 1회차를 **올인원으로 시작하는가?** | 9.1 모드 선택. 빠르지만 검증 부담이 4배 |
