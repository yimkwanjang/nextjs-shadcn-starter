...
> 상태: **Draft** — 9절 열린 질문이 확정되면 PRD로 승격한다.
> 작성일: 2026-08-31
...
| | 상세페이지 | 숏츠 | 블로그 |
|---|-----------|------|-------|
| 축 | 스크롤(공간) | 시간(초) | 읽기 순서(스크롤) |
| 1 훅 | 최상단 GIF | 0~2초 | 제목 + 첫 문장 |
| 2 공감 | 1스크롤 | 2~6초 | 서론 (경험담) |
| 3 해결책 | 2스크롤 | 6~12초 | 본문 → 해결책 |
| 4 증거 | 3스크롤 | 12~18초 | 직접 써본 후기·사진 |
| 5 혜택 | 4스크롤 | 18~26초 | The Bridge (제품 침투) |
| 6 행동 | 하단 고정 CTA | 26~30초 | 내부 링크 (정보 연장선) |

→ **결론: 시퀀스를 한 번 만들고, 렌더러를 여러 개 둔다.** 기획을 채널 수만큼 반복하지 않는 것이
이 시스템의 존재 이유다. 채널별로 따로 만들면 메시지가 갈라지고 자동화 이득의 절반이 사라진다.

단 블로그는 **방향이 반대(Pull)** 라서 앞뒤에 레이어가 하나씩 더 붙는다 → 5절.
...
2. `PROOF` 단계의 `visual.humanRequired`는 **항상 `true`**. 리뷰·인증·판매량을
   AI가 지어내면 그건 자동화가 아니라 허위광고다(10절 참조).
...
| Q-10 | **영상 트랙과 블로그 트랙 중 무엇을 먼저 굴리는가?** | 6절 순서. 블로그가 더 싸고 빠르지만 성과 확인에 2주 필요 |
